...
export const ThemeSelection: React.FC<ThemeSelectionProps> = ({ onSelectTheme }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-50">
      ...
      {/* Compact Header Area */}
      <div className="px-3 pt-3 pb-2 md:px-6 md:pt-6 shrink-0 bg-white border-b border-slate-100 z-10">
          <div className="flex justify-between items-center mb-3">
              <h2 className="text-lg md:text-2xl font-black text-brand-blue flex items-center gap-2">
                   <Rocket size={18} className="text-brand-orange" strokeWidth={3} />
                   Today's Mission
              </h2>
              ...
              <div className="flex bg-slate-100 p-1 rounded-full">
                {[3, 5, 7].map(num => (
                  <button
                    key={num}
                    onClick={() => { playClick(); setWordCount(num); }}
                    className={`px-3 py-1 rounded-full font-bold text-xs md:text-sm transition-all ${
                      wordCount === num 
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-6 scrollbar-hide relative">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 pb-20">
              {THEMES.filter(t => t.category === activeTab).map(theme => (
                  <button
                      key={theme.id}
                      onClick={() => { playClick(); onSelectTheme(theme, wordCount); }}
      ...
      {/* Floating Surprise Button (Mobile & Desktop unified for simplicity on tight layout) */}
                ...
                <div className="bg-slate-50 p-6 rounded-[1.8rem] border border-slate-100 flex flex-col items-center text-center">
                    ...
                    {randomTheme ? (
                        <div className={`w-full p-6 rounded-2xl mb-6 bg-white border-2 border-dashed border-slate-200 shadow-sm relative overflow-hidden`}>
                            ...
                            <p className="text-sm text-slate-500 font-medium leading-tight">{randomTheme.description}</p>
                        </div>
                    ) : (
