...
export const StoryQuiz: React.FC<StoryQuizProps> = ({ words, onComplete }) => {
  ...
  useEffect(() => {
    // Play audio when question appears
    if (!showSuccess) {
        audioManager.playTTS(`Where is ${currentTarget.word}?`);
    }
    ...
    if (wordId === currentTarget.id) {
        ...
        setTimeout(() => {
            if (currentIndex < words.length - 1) {
                ...
                playFanfare(); // Sound
                setShowSuccess(true);
                audioManager.playTTS("Excellent work! You passed the quiz!");
            }
        }, 1500);
        ...
        setIsCorrect(false);
        playError(); // Sound
        audioManager.playTTS("Try again!");
        setTimeout(() => {
            setSelectedAnswer(null);
            setIsCorrect(null);
        }, 1000);
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 relative overflow-hidden">
        ...
        <div className="p-6 md:p-8 text-center shrink-0">
            ...
            <button 
                onClick={() => { playClick(); audioManager.playTTS(currentTarget.word); }}
