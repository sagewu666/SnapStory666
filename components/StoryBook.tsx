...
import { playClick, playSuccess, playError, playPop, playPageTurn, playFanfare } from '../utils/soundUtils';
...
export const StoryBook: React.FC<StoryBookProps> = ({ story, onFinish, kidProfile, onWordAdded }) => {
  ...
  const [isGeneratingAudio, setIsGeneratingAudio] = useState(false); // Loading state
  ...
  const [dictModal, setDictModal] = useState<{ isOpen: boolean, word: string, loading: boolean, data: any | null, isAdding: boolean }>({ isOpen: false, word: '', loading: false, data: null, isAdding: false });
  ...
  const stopAllAudio = () => {
      ...
      // 2. Stop Browser TTS (just in case)
  ...
  const playWordAudio = async (text: string) => {
      // Robust audio player with fallback
      try {
          const audioData = await generateSpeech(text);
          if (audioData) {
              await playGeminiAudioData(audioData, []); 
          } else {
             // Fallback to browser TTS if API returns null/500
  ...
  const playHumanNarration = async () => {
      ...
      try {
          ...
          const audioData = await generateSpeech(text);
          setIsGeneratingAudio(false);
          if (isMountedRef.current && audioData) {
             // Pass word segments for sync
             playGeminiAudioData(audioData, wordsOnPage);
          }
      } catch (err) {
  ...
  const handleWordClick = async (e: React.MouseEvent, wordText: string, isValidInteractive: boolean) => {
    ...
    // Improved cleaning: strip punctuation from start/end but keep internal apostrophes/hyphens
    // e.g. "apple." -> "apple", "don't" -> "don't", "blue-jay" -> "blue-jay"
    ...
    if (isValidInteractive) {
        ...
        const match = story.learnedWords?.find(lw => lw.word.toLowerCase() === cleanWord.toLowerCase());
        if (match) {
            playPop();
            playWordAudio(cleanWord);
            setHandAnim({ x: e.clientX, y: e.clientY, show: true });
            if (!foundWordsMap[currentPage]?.includes(match.id)) {
                setFoundWordsMap(prev => ({
                    ...prev,
                    [currentPage]: [...(prev[currentPage] || []), match.id]
                }));
            }
            setTimeout(() => {
                if (isMountedRef.current) {
                    setHandAnim(prev => ({ ...prev, show: false }));
                    setSelectedWordData(match);
                    setIsSpelling(false);
                }
            }, 1000);
        }
    } else {
        // Handle DICTIONARY Click (New Feature)
        playClick();
        stopAllAudio();
        
        // Open the dictionary modal.
        setDictModal({ isOpen: true, word: cleanWord, loading: true, data: null, isAdding: false });
  ...
  const handleAddToLibrary = async () => {
      ...
      try {
          // Generate an illustration for the card
          const illustration = await generateIllustration(
              dictModal.data.visualDetail, 
              "icon sticker style, simple, white background", 
              "cartoon"
          );
          ...
          const newWord: LearnedWord = {
              ...
              imageUrl: illustration || 'https://cdn-icons-png.flaticon.com/512/3574/3574069.png', // Fallback
              originalImage: illustration || '',
              timestamp: Date.now(),
    ...
    return (
        <div className="flex flex-wrap gap-x-2 md:gap-x-3 gap-y-3 md:gap-y-5 leading-loose md:leading-[2.5] justify-center md:justify-start px-2 py-4">
            {wordsOnPage.map((item, idx) => {
                const cleanText = item.text.replace(/[^a-zA-Z]/g, '');
                const matchingLearnedWord = story.learnedWords?.find(w => w.word.toLowerCase() === cleanText.toLowerCase());
                const isValidInteractive = item.isSpecial && !!matchingLearnedWord;
                const isFound = isValidInteractive && matchingLearnedWord && foundWordsMap[currentPage]?.includes(matchingLearnedWord.id);
  ...
  const pageTargets = useMemo(() => {
      if (!story.learnedWords) return [];
      return story.learnedWords.filter(lw => {
          const regex = new RegExp(`\\b${lw.word}\\b`, 'i');
          return regex.test(currentContent.text);
      });
  }, [currentContent, story.learnedWords]);
  ...
  return (
    <div className="h-full w-full flex flex-col bg-white md:bg-transparent relative">
      ...
      {/* DICTIONARY MODAL */}
      {dictModal.isOpen && (
         <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-pop-in">
             <div className="bg-white rounded-[2.5rem] w-full max-w-sm shadow-2xl relative border-8 border-white ring-4 ring-brand-purple flex flex-col overflow-hidden">
                 
                 <button onClick={() => setDictModal(prev => ({...prev, isOpen: false}))} className="absolute top-4 right-4 bg-slate-100 p-2 rounded-full hover:bg-slate-200 z-10">
                     <X size={20} className="text-slate-500" />
                 </button>

                 <div className="bg-purple-50 p-8 text-center border-b border-purple-100 flex flex-col items-center">
                     {dictModal.loading ? (
                         <div className="h-20 flex items-center justify-center"><Loader2 className="animate-spin text-brand-purple" size={40} /></div>
                     ) : (
                         <>
                            <div className="text-6xl mb-4 animate-bounce-slow">{dictModal.data?.emoji || "✨"}</div>
                            <h2 className="text-4xl font-black text-brand-purple capitalize tracking-tight mb-2">{dictModal.word}</h2>
                            <button onClick={() => playWordAudio(dictModal.word)} className="bg-white px-4 py-2 rounded-full shadow-sm flex items-center gap-2 text-brand-purple font-bold hover:scale-105 transition-transform">
                     ...
                     {dictModal.loading ? (
                         ...
                         <div className="text-center">
                             ...
                             >
                                 {dictModal.isAdding ? (
                                     <>
                                        <Loader2 className="animate-spin" /> Creating Sticker...
                                     </>
                                 ) : (
                                     <>
      ...
      {selectedWordData && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-pop-in">
             <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl relative border-4 border-white ring-4 ring-brand-blue flex flex-col max-h-[90dvh] overflow-hidden">
                ...
                <div className="p-4 md:p-8 overflow-y-auto flex-1 flex flex-col items-center">
                    ...
                    {!isSpelling ? (
                         <div className="text-center w-full">
                             ...
                             <p className="text-slate-500 font-medium text-base md:text-lg mb-6">"{selectedWordData.definition}"</p>
      ...
      {/* STORY HEADER */}
      <div className="h-14 md:h-16 flex items-center justify-between px-3 md:px-6 shrink-0 bg-white border-b border-slate-100 z-30 shadow-sm">
          ...
          <div className="flex items-center gap-2">
              ...
              >
                  ...
                  <span className="text-xs font-bold uppercase">{isGeneratingAudio ? 'Loading...' : 'Storyteller'}</span>
              </button>
          </div>
      </div>

      {/* MAIN LAYOUT SPLIT */}
      <div className="flex-1 flex flex-col md:flex-row h-full min-h-0 relative overflow-hidden">
         ...
         <div className="flex-1 w-full md:w-1/2 flex flex-col bg-white relative h-full min-h-0">
             ...
             <div className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-slate-100 p-2 md:p-4 z-20 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] flex flex-col gap-3">
                  <div className="w-full overflow-x-auto pb-1 scrollbar-hide">
                      <div className="flex gap-4 md:justify-start px-2 min-w-max">
                        <span className="text-[10px] font-bold text-slate-300 uppercase writing-mode-vertical self-center hidden md:block">Find:</span>
                        {pageTargets.length > 0 ? pageTargets.map(item => {
                            const isFound = foundWordsMap[currentPage]?.includes(item.id);
                            return (
                                <div key={item.id} className={`flex flex-col items-center gap-1 transition-all duration-500 ${isFound ? 'opacity-100' : 'opacity-60 grayscale'}`}>
                                    <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl overflow-hidden border-2 ${isFound ? 'border-brand-green ring-2 ring-green-100' : 'border-slate-200'}`}>
                                        <img src={item.imageUrl} className="w-full h-full object-cover" alt={item.word} />
                                    </div>
                                    <span className={`text-[9px] font-bold uppercase max-w-[50px] truncate ${isFound ? 'text-brand-green' : 'text-slate-400'}`}>{item.word}</span>
                                </div>
                            );
                        }) : (
                            <div className="text-xs text-slate-400 italic py-2 w-full text-center">Enjoy reading!</div>
                        )}
                      </div>
                  </div>
